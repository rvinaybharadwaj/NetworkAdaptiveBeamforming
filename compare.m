...
d=lamb/4; %used below
%arrays to evaluate
arryshort=[lamb/4,lamb/4,  0,lamb/4,   lamb/4,0;... %lamb/4 square array
%    0,lamb/4,   0,2*lamb/4,   0,3*lamb/4;...        %lamb/4 ULA
%    lamb/2,lamb/2,  0,lamb/2,   lamb/2,0;...        %lamb/2 square array
%    0,lamb/2,   0,2*lamb/2,   0,3*lamb/2;...           %lamb/2 ULA  
    d*cos(pi/6),d*sin(pi/6),   d*cos(5*pi/6),d*sin(5*pi/6),    0,-d;... %Balanis Y
     -0.0191   -0.0919    0.0505   -0.1546    0.0695   -0.0627;...
 ];


leg=['sqr/4';...                        %define legend for plot results 
    %'ULA/4';'sqr/2';'ULA/2';... 
    'BalaY'; ...
    'satee';...
...
DESIRED(center-beamwidth/2:center+beamwidth/2)=ones(1,beamwidth+1)*wt;
...
PerfType='NSR';
response=plotresponse(arryshort);  %plot comparison
...
for karry=1:Narray
    ...
    PerfType='MSE';   %the NSR unconstrained version doesnt work!
